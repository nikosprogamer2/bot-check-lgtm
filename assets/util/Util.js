...
module.exports = {
    async play(song, message, quiet = false) {
        ...
        const dispatcher = queue.connection
            ...
            .on("finish", () => {
                ...
                if (queue.loop && queue.songs.length > 0) {
                    ...
                    // check if next song is same as last played and set to true/false the quiet definition
        ...
        if (quiet) return; // stops play message when looped

        if (!quiet) { // send message only when not looped
